/**
 * Baut den MCP-Server mit allen Tools. Von beiden Transporten genutzt:
 * index.ts (stdio, lokal) und http.ts (Streamable HTTP, remote/vServer).
 *
 * Handelt via persönlichem API-Token (ENV DODIDIS_API_TOKEN) als der jeweilige
 * Nutzer — mit dessen Rollen/Rechten. Lesen (Fragen beantworten) + Schreiben
 * (Videos/Projekte eintragen, Status, Termine, Posting). Die Video-DATEI selbst
 * wird bewusst NICHT hochgeladen (bleibt Browser→NAS). Das generische Fallback-
 * Tool (tool_api_call) erreicht jeden erlaubten Endpunkt.
 */
...
// die Posting-Erweiterung — sonst weist das Backend 'gepostet' sauber mit 400 ab.
const STATUS = [
...
/** Erzeugt einen frischen MCP-Server mit allen registrierten Tools. */
